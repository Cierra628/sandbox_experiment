...
OpenClaw 的主要瓶颈不是本地超分计算，而是 VMM 内启动 Node/OpenClaw 时对大量小文件执行 metadata、插件和配置初始化。VirtioFS `cache=never` 最慢；virtio-blk 最快但把约 5.9 s 的设备/rootfs 准备成本放进 `start`。导师提出的 union 方案已经功能验证通过，在保留 VirtioFS 只读 lower 和镜像共享语义的同时，把 `/app` 的可写 upper/work 与状态目录放到 virtio-blk，是当前较平衡的方案。
...
3. **union**：`start` 约 70 ms，`Gateway ready` 约 7.7 s；比直接 block 的 Agent sample 慢约 1.5 s，但保留 VirtioFS lower 的只读复用/共享语义，功能 smoke 和 workload 均通过。
...
5. **建议**：向导师汇报时把 union 定义为“当前平衡的原型方案”，把直接 virtio-blk 作为性能上界，把 `cache=metadata` 作为低改动基线；后续重点测常驻 Agent、并发和长时间稳定性。
...
因此，本报告后续的 union 实验不是替换已有基线，而是验证导师提出的更接近生产语义的组合：VirtioFS 只读 lower、virtio-blk 提供 writable upper/work，并在 guest 内形成 OverlayFS merged view。
...
每轮创建全新容器，要求 Agent 调用一次本地 `image_upscale` 工具，将 32×32 PGM 放大到 64×64（1024→4096 像素），并返回 `KUASAR_SAMPLE_OK`。三轮均满足：
...
相对 VirtioFS `cache=never`，混合方案将 Agent sample exec wall 降低约 **81.9%**，Agent internal 降低约 **72.7%**。工具本身只有约 115 ms，远小于 Agent exec wall，因此主要开销来自 CLI/Agent 初始化、文件访问、模型交互和 CRI exec，而不是超分计算。
...
为验证导师提出的“VirtioFS 只读 lower + virtio-blk writable upper/work”是否能直接由容器完成，我们准备了独立的 ext4 upper/work 镜像（`/dev/loop25`），并以 `kuasar-vmm` 启动一个只用于能力探测的容器。测试设计为：
...
当前 union 版本相对直接 `/app` virtio-blk 的平均差异为：Gateway ready +344 ms、Agent sample exec +1532 ms、Agent internal +626 ms；但两者仍保持相同数量级的 VMM 启动成本，且 union 保留了原始 VirtioFS lower 的只读语义。工具本身仍约 115 ms，差异主要来自 union/upper 路径上的应用初始化和 Agent 外层执行，而非超分计算。
...
union 的复用表现接近 VirtioFS metadata cache，同时保留了 guest 内 OverlayFS 的 upper/work 隔离语义；相较直接 block `/app`，当前实现仍有约 1.5 s 的平均每请求差距，主要来自 `/app` union 层和 Agent/CLI 外层，而不是 image_upscale 工具本身。

## 10. 结论与边界

1. **混合挂载方案功能正确**：直接 block `/app` + state 和 union `/app` + state 均 3/3 轮通过。
2. **导师提出的 union 语义已经实现并验证**：VMM 保持 VirtioFS lower，guest 内创建 OverlayFS merged `/app`，virtio-blk 只承载 upper/work 和可写 state。
3. **启动成本没有退化到全 virtio-blk**：union 的 `start` 约 70 ms，而全 virtio-blk 基线约 5.9 s；因此避免了将完整 rootfs 设备准备成本放到 VMM 启动阶段。
4. **union 当前不是直接 block `/app` 的无损替代**：本轮数据中 Gateway/Agent 指标比直接 block `/app` 略慢，但仍远快于未优化 VirtioFS 全路径；是否值得采用取决于对 lower 只读复用、镜像共享和 upper 写时隔离的需求。
