...
RUNS="${HYBRID_STATE_RUNS:-3}"
READY_TIMEOUT="${HYBRID_READY_TIMEOUT:-180}"
RESULT_DIR="${HYBRID_STATE_RESULT_DIR:-$ROOT_DIR/.artifacts/hybrid-state-$(date -u +%Y%m%dT%H%M%SZ)}"

usage() {
  printf '%s\n' \
    'Usage: scripts/21-benchmark-hybrid-state.sh [options]' \
    '' \
    'Runs kuasar-vmm with VirtioFS rootfs and an extra virtio-blk state mount.' \
    '' \
    'Options:' \
    '  --env-file FILE    source hybrid-state.env from script 20' \
    '  --state-loop DEV   existing loop block device' \
...
if [ -n "$ENV_FILE" ]; then
  ...
  . "$ENV_FILE"
  set +a
  STATE_LOOP_DEVICE="${HYBRID_STATE_LOOP_DEVICE:-$STATE_LOOP_DEVICE}"
fi

[[ "$RUNS" =~ ^[1-9][0-9]*$ ]] || { echo 'error: --runs must be positive' >&2; exit 2; }
[[ "$READY_TIMEOUT" =~ ^[1-9][0-9]*$ ]] || { echo 'error: --ready-timeout must be positive' >&2; exit 2; }
[ -n "$STATE_LOOP_DEVICE" ] || { echo 'error: state loop device is required' >&2; exit 2; }

printf '%s\n' \
  'Hybrid state benchmark plan:' \
  '  rootfs=VirtioFS' \
  '  state=/home/node/.openclaw on virtio-blk' \
  '  handler=kuasar-vmm' \
...
HYBRID_CONTAINER_SPEC="$RESULT_DIR/specs/openclaw-container-hybrid-state-vmm.json"
jq --arg state "$STATE_LOOP_DEVICE" '
  (.mounts[] | select(.container_path == "/home/node/.openclaw")).host_path = $state |
  .metadata.name = "openclaw-hybrid-state" |
...
run_once() {
  ...
  local runp_begin create_begin start_begin
  ...
  jq --arg uid "hybrid-state-$run-$(date +%s%N)" --argjson attempt "$run" \
    '.metadata.uid=$uid | .metadata.attempt=$attempt' \
    "$BASE_POD_SPEC" > "$pod_spec"
  jq --arg log "hybrid-state-$run-$(date +%s%N).log" --argjson attempt "$run" \
  ...
  if [ "$status" = PASS ]; then
    ...
    if health_output="$("${CRI[@]}" exec "$cid" node openclaw.mjs gateway health --json 2>&1)"; then
      printf '%s\n' "$health_output" > "$RESULT_DIR/hybrid-state-run${run}-health.raw"
  ...
  [ -z "$cid" ] || "${CRI[@]}" logs "$cid" > "$RESULT_DIR/hybrid-state-run${run}.log" 2>&1 || true
  ...
  jq -n \
    ...
    '{handler:$handler,config:$config,run:$run,status:$status,note:$note,state_loop:$state_loop,cri_ready_ms:$cri_ready_ms,runp_ms:$runp_ms,create_ms:$create_ms,start_ms:$start_ms,gateway_ready_ms:$gateway_ready_ms,health_exec_ms:$health_exec_ms,health_internal_ms:$health_internal_ms,cleanup_ms:$cleanup_ms,total_ms:$total_ms}' \
...
jq '
  ...
  {
    handler: "kuasar-vmm",
    config: "virtiofs-root+virtio-blk-state",
    runs: length,
