...
RUNS="${UNION_REUSE_RUNS:-3}"
REQUESTS="${UNION_REUSE_REQUESTS_PER_CONTAINER:-5}"
PASSES="${IMAGE_WORKLOAD_PASSES:-512}"
READY_TIMEOUT="${READY_TIMEOUT:-180}"
ARTIFACT_TIMEOUT="${MODEL_SAMPLE_ARTIFACT_TIMEOUT:-15}"
LABEL="overlay-app-virtio-blk-upper+virtio-blk-state"
...
RESULT_DIR="${RESULT_DIR:-$ROOT_DIR/.artifacts/union-container-reuse-$(date -u +%Y%m%dT%H%M%SZ)}"
VMM_CONFIG="${VMM_CONFIG:-/etc/openclaw-kuasar/vmm.toml}"

usage() {
  printf '%s\n' \
    'Usage: scripts/30-benchmark-union-reuse.sh --state-loop DEV --upper-loop DEV [--runs N] [--requests N] [--passes N] [--result-dir DIR] [--dry-run]' \
    '' \
    'Runs image_upscale repeatedly in one fresh union-overlay VMM container per sequence.'
}
...
for value_name in RUNS REQUESTS PASSES READY_TIMEOUT ARTIFACT_TIMEOUT; do
  value="${!value_name}"
  [[ "$value" =~ ^[1-9][0-9]*$ ]] || {
...
printf '%s\n' \
  'Union container reuse benchmark plan:' \
  '  handler=kuasar-vmm' \
  "  rootfs_config=$LABEL" \
  ...
  "  passes=$PASSES" \
  '  /app=overlayfs (VirtioFS lower + virtio-blk upper/work)' \
  '  /home/node/.openclaw=virtio-blk' \
...
if ! sudo grep -Eq '^[[:space:]]*container_storage_backend[[:space:]]*=[[:space:]]*"virtiofs"' "$VMM_CONFIG"; then
  echo "error: $VMM_CONFIG must use container_storage_backend=\"virtiofs\" for union" >&2
  exit 1
fi
cri() {
  sudo crictl \
...
UNION_CONTAINER="$RESULT_DIR/specs/container-template.json"
UNION_JSON='{"target":"/app","upper_mount":"/tmp","upper_dir":"upper","work_dir":"work","lower_debug_mount":"/app.lower"}'
jq --arg state "$STATE_LOOP_DEVICE" --arg upper "$UPPER_LOOP_DEVICE" --arg union "$UNION_JSON" \
  '.mounts |= map(if .container_path == "/home/node/.openclaw" then .host_path = $state else . end) | .mounts += [{"container_path":"/tmp","host_path":$upper,"readonly":false}] | .annotations = ((.annotations // {}) + {"io.openclaw.overlayfs.union":$union}) | .labels = ((.labels // {}) + {"io.openclaw.overlayfs.union":$union}) | .envs = ((.envs // []) + [{"key":"OPENCLAW_KUASAR_OVERLAY_UNION","value":$union}]) | .metadata.name="openclaw-union-reuse" | .linux.security_context.run_as_user.value=0 | .linux.security_context.run_as_group.value=0' \
  "$BASE_CONTAINER" > "$UNION_CONTAINER"
python3 -m json.tool "$UNION_CONTAINER" >/dev/null
...
for run in $(seq 1 "$RUNS"); do
  ...
  jq \
    ...
    "$UNION_CONTAINER" > "$container_spec"
  ...
  cri exec "$CONTAINER_ID" \
    ...
    /tmp \
    > "$run_dir/mounts.txt"
  if ! grep -Eq 'overlayfs /app|overlay /app' "$run_dir/mounts.txt" || \
     ! grep -Eq 'fuseblk /app.lower|virtiofs' "$run_dir/mounts.txt" || \
     ! grep -Eq 'ext[234] /home/node/.openclaw|ext[234] /tmp' "$run_dir/mounts.txt"; then
    echo "error: union mount verification failed in run $run" >&2
    exit 1
  fi
