...
UPPER_LOOP_DEVICE="${HYBRID_UNION_UPPER_LOOP_DEVICE:-}"
...
RESULT_DIR="${HYBRID_IMAGE_RESULT_DIR:-$ROOT_DIR/.artifacts/union-image-workload-$(date -u +%Y%m%dT%H%M%SZ)}"

usage() {
  printf '%s\n' \
    'Usage: scripts/29-benchmark-union-image-workload.sh [options]' \
    '' \
    'Runs the image-upscale Agent sample with VirtioFS /app lower + virtio-blk overlay upper/work and virtio-blk state.' \
    '' \
    'Options:' \
    '  --state-loop DEV       existing state loop block device' \
    '  --upper-loop DEV       existing writable overlay upper/work loop device' \
...
[ -n "$UPPER_LOOP_DEVICE" ] || { echo 'error: overlay upper loop device is required' >&2; exit 2; }

printf '%s\n' \
  'Overlay union image workload benchmark plan:' \
  '  rootfs=VirtioFS' \
  '  /app lower on VirtioFS + overlay upper/work on virtio-blk' \
...
jq --arg state "$STATE_LOOP_DEVICE" --arg upper "$UPPER_LOOP_DEVICE" \
  ...
  .mounts += [{"container_path":"/tmp","host_path":$upper,"readonly":false}] |
  .annotations["io.openclaw.overlayfs.union"] = $union |
  .labels["io.openclaw.overlayfs.union"] = $union |
  .envs += [{"key":"OPENCLAW_KUASAR_OVERLAY_UNION","value":$union}] |
  .metadata.name = "openclaw-union-image-workload" |
...
capture_mounts() {
  ...
  "${CRI[@]}" exec "$cid" sh -c \
    'printf "%s\\n" "--- id ---"; id; printf "%s\\n" "--- fs types ---"; stat -f -c "%T %n" / /app /app.lower /home/node/.openclaw /tmp; printf "%s\\n" "--- mountinfo ---"; grep -E " /app(\.lower)? | /home/node/.openclaw | /tmp " /proc/self/mountinfo || true' \
...
verify_workload() {
  local cid="$1"
  "${CRI[@]}" exec "$cid" sh -c \
    'test -f /app/openclaw.mjs && (test -f /app/dist/entry.js || test -f /app/dist/entry.mjs) && test -s /home/node/.openclaw/workspace/complex-workload/input.pgm && test -f /home/node/.openclaw/workspace/.openclaw/extensions/image-upscale/image-upscale.mjs'
}
...
run_once() {
  ...
  local sample_raw="$RESULT_DIR/union-image-run${run}-sample.raw"
  local sample_stderr="$RESULT_DIR/union-image-run${run}-sample.stderr"
  local artifact="$RESULT_DIR/union-image-run${run}-tool.json"
  local artifact_stderr="$RESULT_DIR/union-image-run${run}-tool.stderr"
  ...
  jq --arg uid "union-image-$run-$(date +%s%N)" --argjson attempt "$run" \
    '.metadata.uid=$uid | .metadata.attempt=$attempt' \
    "$BASE_POD_SPEC" > "$pod_spec"
  jq --arg log "union-image-$run-$(date +%s%N).log" --argjson attempt "$run" \
  ...
  if [ -n "$cid" ]; then
    capture_mounts "$cid" "$RESULT_DIR/union-image-run${run}-mounts.txt"
  fi
  ...
  if [ "$status" = PASS ]; then
    ...
    if health_output="$("${CRI[@]}" exec "$cid" node openclaw.mjs gateway health --json 2>&1)"; then
      printf '%s\n' "$health_output" > "$RESULT_DIR/union-image-run${run}-health.raw"
  ...
  if [ "$status" = PASS ]; then
    ...
    if sample_output="$("${CRI[@]}" exec "$cid" node openclaw.mjs agent --local --agent main \
      --session-key "agent:main:union-image-$run-$(date +%s%N)" \
  ...
  [ -z "$cid" ] || "${CRI[@]}" logs "$cid" > "$RESULT_DIR/union-image-run${run}.log" 2>&1 || true
  ...
  jq -n \
    ...
    --arg config virtiofs-root+overlay-app-virtio-blk-upper+virtio-blk-state \
    --argjson run "$run" \
    --arg status "$status" \
    --arg note "$note" \
...
jq '
  ...
  {
    handler: "kuasar-vmm",
    config: "virtiofs-root+overlay-app-virtio-blk-upper+virtio-blk-state",
    runs: length,
